...
# Checking for non-stationarity: differenced data
adf.test(p3.data.diff)
pp.test(p3.data.diff)

# Time plot: differenced data
plot(p3.data.diff, type = "b", main = "Dataset 3: Lag-1 Difference", xlab = "Time", ylab = "Data", cex = 0.5)

# Comparing segment of differenced data to original data
par(mfrow=c(2,1))
...
### PLOT SHOWING EXACT RELATIONSHIP
